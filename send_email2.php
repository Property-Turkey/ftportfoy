...
if(empty($error_msg)){
    ...
    try {
        ...
        $mail->Host       = 'mail.ftportfoy.com';                     //Set the SMTP server to send through
        $mail->SMTPAuth   = true;                                   //Enable SMTP authentication
        $mail->Username   = 'noreply@ftportfoy.com';                     //SMTP username
        $mail->Password   = '--Password--';                               //SMTP password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
        $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

        //Recipients
        $mail->setFrom('noreply@ftportfoy.com', 'FT Portfoy');
        $mail->addAddress('ft@ftportfoy.com');     //Add a recipient
